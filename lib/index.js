...
module.exports = function(config, publisher) {
  ...
  contractAtAddress = (function(_this) {
    return function(source, name, address, cb) {
      ...
      return web3.eth.contract(abi).at(address, cb);
    };
  })(this);
  ...
  buildGenericMethods = function(contract_schema) {
    var Contracts, EventDecoders, abis, contracts;
    Contracts = {};
    abis = [];
    contracts = [];
    EventDecoders = {};
    Object.keys(contract_schema).map(function(name) {
      ...
      Contracts[name] = {
        ...
        compile: function(cb) {
          ...
          return cb(null, compiled.contracts[name]);
        }
      };
      console.log('Compiling event decoders...', name);
      EventDecoders[name] || (EventDecoders[name] = {});
      return Contracts[name].compile(function(err, compiled) {
        ...
        return abi.map(function(abi_fn) {
          var hashed_signature, input_names, input_types, signature;
          if (abi_fn.type === 'event') {
            input_types = abi_fn.inputs.map(function(i) {
              return i.type;
            });
            input_names = abi_fn.inputs.map(function(i) {
              return i.name;
            });
            signature = abi_fn.name + "(";
            abi_fn.inputs.map(function(i) {
              return signature += i.type + ',';
            });
            signature = signature.slice(0, -1) + ')';
            hashed_signature = web3.sha3(signature);
            return EventDecoders[name][hashed_signature] = function(e) {
              var data, result;
              data = SolidityCoder.decodeParams(input_types, e.data.replace("0x", ""));
              result = {};
              input_names.map(function(i_n, i) {
                return result[i_n] = data[i];
              });
              return result;
            };
          }
    ...
    return {
      ...
      decodeEvent: function(name, event) {
        var ref;
        if (EventDecoders[name] != null) {
          if (EventDecoders[name][(ref = event.topics) != null ? ref[0] : void 0] != null) {
